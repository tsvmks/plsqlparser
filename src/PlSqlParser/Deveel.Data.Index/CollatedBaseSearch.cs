...
namespace Deveel.Data.Index {
	...
	public abstract class CollatedBaseSearch : SelectableScheme {
		protected CollatedBaseSearch(ITable table, int column)
			: base(table, column) {
		}
		...
		/// column.
		/// </returns>
		protected abstract int SearchFirst(DataObject val);
		...
		/// in the column.
		/// </returns>
		protected abstract int SearchLast(DataObject val);

		/// <summary>
		/// Gets the size of the set (the number of rows in this column).
		/// </summary>
		protected virtual long SetSize {
		...
		/// </remarks>
		protected virtual DataObject FirstInCollationOrder {
		...
		/// </remarks>
		protected virtual DataObject LastInCollationOrder {
		...
		/// range of integers.
		/// </returns>
		protected virtual IList<long> AddRangeToSet(long start, long end, IList<long> list) {
			if (list == null)
		...
		/// </returns>
		private IList<long> AddRange(SelectableRange range, IList<long> list) {
		...
		public override IList<long> SelectRange(SelectableRange range) {
			...
			IList<long> list = AddRange(range, null);
			if (list == null)
				return new List<long>(0);
