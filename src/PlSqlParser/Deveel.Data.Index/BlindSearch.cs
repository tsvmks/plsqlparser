...
namespace Deveel.Data.Index {
	...
	public sealed class BlindSearch : SelectableScheme {
		public BlindSearch(ITable table, int column)
			: base(table, column) {
		}
		...
		private void DoInsertSort(IList<long> list, long row) {
			int listSize = list.Count;
			if (listSize == 0) {
				list.Add(row);
			} else {
				long point = HighestSearch(GetCellContents(row), list, 0, listSize - 1);
				if (point == listSize) {
					list.Add(row);
				} else {
					list.Insert((int)point, row);
				}
			}
		}
		...
		public override IList<long> SelectAll() {
			List<long> rowList = new List<long>((int)Table.RowCount);
			IEnumerator<long> e = Table.GetRowEnumerator();
			while (e.MoveNext()) {
				DoInsertSort(rowList, e.Current);
			}
			return rowList;
		}

		/// <inheritdoc/>
		public override IList<long> SelectRange(SelectableRange range) {
		...
		sealed class RangeChecker {
			...
			// The DataObject objects to check against.
			...
			private void SetupRange(int i, SelectableRange range) {
				DataObject l = range.Start;
				RangePosition lf = range.StartPosition;
				DataObject u = range.End;
			...
			public IList<long> Resolve() {
				...
				while (e.MoveNext()) {
					...
					for (int i = 0; i < size; ++i) {
						...
						if (lf != NoCheck) {
							++compare_tally;
							DataObject v = scheme.GetCellContents(row);
