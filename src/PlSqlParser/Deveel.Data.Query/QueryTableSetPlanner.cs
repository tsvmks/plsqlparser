...
namespace Deveel.Data.Query {
	...
	internal sealed class QueryTableSetPlanner {
		...
		/// </summary>
		private bool HasJoinOccured { get; set; }
		...
		/// <returns></returns>
		private PlanTableSource FindTableSource(ObjectName reference) {
		...
		/// </para>
		/// </remarks>
		private void EvaluateSingles(IEnumerable<Expression> singleVars, IList<ExpressionPlan> evaluateOrder) {
			...
			foreach (Expression andexp in singleVars) {
				...
				if (op.IsSubQuery()) {
					...
					IList<ObjectName> allVars = exps[0].AllVariables().ToList();
					if (allVars.Count == 0) {
					...
					// Simple LHS?
					ObjectName v = exps[0].AsVariable();
					if (v != null) {
		...
		/// search must be used.
		/// </remarks>
		private void EvaluateMultiples(IEnumerable<Expression> multiVars, IList<ExpressionPlan> evaluateOrder) {
			// FUTURE OPTIMIZATION:
			...
			foreach (Expression expr in multiVars) {
				...
				ObjectName lhsVar = exps[0].AsVariable();
				ObjectName rhsVar = exps[1].AsVariable();
		...
		/// expression.
		/// </remarks>
		private void EvaluateSubLogic(IEnumerable<Expression> sublogicExprs, IList<ExpressionPlan> evaluateOrder) {
			//each_logic_expr:
			foreach (Expression expr in sublogicExprs) {
				// Break the expression down to a list of OR expressions,
				IList<Expression> orExprs = expr.BreakByOperator(new List<Expression>(), Operator.Or);
				...
				foreach (Expression orExpr in orExprs) {
					IList<ObjectName> vars = orExpr.AllVariables().ToList();
		...
		private void PlanForExpressionList(IEnumerable<Expression> andList) {
			var subLogicExpressions = new List<Expression>();
			// The list of expressions that have a sub-select in them.
			var subQueryExpressions = new List<Expression>();
			...
			var patternExpressions = new List<Expression>();
			...
			var singleVars = new List<Expression>();
			// The list of multi variable expressions (possible joins)
			var multiVars = new List<Expression>();
			...
			foreach (Expression andexp in andList) {
				...
				// '[expression] = true'
				if (binary == null || 
					binary.Operator.IsMathematical()) {
					exp = Expression.Equal(exp, Expression.Constant(DataObject.BooleanTrue));
					op = Operator.Equal;
				} else {
					op = binary.Operator;
				}
				...
				if (op.IsLogical()) {
					...
					IList<ObjectName> vars = andexp.AllVariables().ToList();
					if (vars.Count == 0) {
		...
		private void PlanForExpression(Expression exp) {
			...
			if (exp is BinaryExpression && 
				((BinaryExpression)exp).Operator.IsLogical()) {
				var binaryExp = (BinaryExpression) exp;
				Operator lastOp = binaryExp.Operator;
				...
				} else if (lastOp == Operator.And) {
					// parsing an AND block
					// The list of AND expressions that are here
					IEnumerable<Expression> andList = CreateAndList(new List<Expression>(), exp);
		...
		/// </remarks>
		/// <returns></returns>
		private static IEnumerable<Expression> CreateAndList(IList<Expression> list, Expression exp) {
		...
		private sealed class SingleVarPlan {
			...
			public ObjectName SingleVariable;
			public ObjectName Variable;
		...
		private class ComplexSingleExpressionPlan : ExpressionPlan {
			...
			public ComplexSingleExpressionPlan(QueryTableSetPlanner qtsp, ObjectName singleVar, Expression expression) {
				this.qtsp = qtsp;
				this.singleVar = singleVar;
				this.expression = expression;
			}
		...
		private class SimplePatternExpressionPlan : ExpressionPlan {
			...
			public SimplePatternExpressionPlan(QueryTableSetPlanner qtsp, ObjectName singleVar, Expression expression) {
				this.qtsp = qtsp;
				this.singleVar = singleVar;
				this.expression = expression;
			}
		...
		private class ExhaustiveSelectExpressionPlan : ExpressionPlan {
			...
			public override void AddToPlanTree() {
				...
				IList<ObjectName> allVars = expression.AllVariables().ToList();
		...
		private class ExhaustiveSubQueryExpressionPlan : ExpressionPlan {
			private readonly QueryTableSetPlanner qtsp;
			private readonly IList<ObjectName> allVars;
			private readonly Expression expression;
		...
		private class SimpleSubQueryExpressionPlan : ExpressionPlan {
			...
			public override void AddToPlanTree() {
				...
				// Update the plan
				tableSource.UpdatePlan(new NonCorrelatedAnyAllNode(leftPlan, rightPlan, new ObjectName[] { leftVar }, op));
			}
		}
		...
		private class StandardJoinExpressionPlan : ExpressionPlan {
			...
			public override void AddToPlanTree() {
				...
				ObjectName lhsVar = exps[0].AsVariable();
				ObjectName rhsVar = exps[1].AsVariable();
				IList<ObjectName> lhsVars = exps[0].AllVariables().ToList();
				IList<ObjectName> rhsVars = exps[1].AllVariables().ToList();
